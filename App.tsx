...
function routine_optimizer(inputs, goalAnalysis, days_remaining) {
  ...
  const goal_hours = Math.min(
    4,
    Math.max(0.5, goalAnalysis.daily_effort_hours || 2)
  );
  ...
  const sleep_hour =
    wake_hour +
    clamped.sleep_hours +
    (24 - clamped.sleep_hours - wake_hour > 16 ? 0 : 0);
  const actual_sleep = 24 - (wake_hour + (24 - clamped.sleep_hours));

  // Workout timing
  const workout_start =
    clamped.workout_timing === "morning"
      ? wake_hour + 0.5
      : clamped.workout_timing === "evening"
      ? 18
      : urgency > 0.5
      ? wake_hour + 0.5
      : 18;
  ...
  return {
    sleep: {
      ...
      sleep_time: formatTime(
        24 - clamped.sleep_hours + wake_hour > 24 ? 22 : wake_hour + 16
      ),
    },
    workout: {
    ...
    hobbies: {
      ...
      note:
        urgency > 0.8 ? "Slightly reduced to focus on goal" : "As preferred",
    },
    work: {
...
function adherence_tracker(history) {
  const streak = history.reduceRight(
    (acc, d) => {
      if (!acc.done && d.adherence >= 0.8)
...
function adaptive_scheduler(
  ...
  const extra_goal = parseFloat(
    (routine.work.goal_work_hours * deficit * 0.5).toFixed(1)
  );
  ...
  return {
    ...
    meta: {
      ...
      difficulty_score: Math.min(
        100,
        routine.meta.difficulty_score + Math.round(urgency_bump * 30)
      ),
      adjustment_note:
        deficit > 0.2
...
Analyze this goal and return ONLY valid JSON (no markdown, no explanation) with this exact structure:
{
  ...
  const res = await fetch("https://api.anthropic.com/v1/messages", {
    ...
    headers: {
      ...
      "x-api-key":
        "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",
...
async function generate_routine_with_ai(
  inputs,
  goalAnalysis,
  routine,
  day_number
...
Return ONLY valid JSON:
{
  ...
  try {
    ...
    return {
      schedule: [],
      daily_focus: "Stay focused.",
      priority_task: "Work on your goal.",
    };
  }
}

async function adapt_plan_with_ai(
  inputs,
  routine,
  adherence,
  missed,
    ...
    >
      ...
      >
        {[
          {
            label: "Sleep",
            val: `${sleep.duration_hours}h`,
            sub: `Wake ${sleep.wake_time}`,
            color: "#6366f1",
          },
          {
            label: "Work",
            val: `${work.duration_hours}h`,
            sub: `+${work.goal_work_hours}h goal`,
            color: "#f59e0b",
          },
          {
            label: "Workout",
            val: `${workout.duration_hours}h`,
            sub: workout.start_time,
            color: "#ef4444",
          },
          {
            label: "Meals",
          ...
          {
            label: "Hobby",
            val: `${hobbies.duration_hours}h`,
            sub: hobbies.note,
            color: "#06b6d4",
          },
          {
            label: "Buffer",
            val: `${meta.free_buffer_hours}h`,
            sub: "free time",
            color: "#6b7280",
          },
        ].map((s) => (
      ...
      <div>
        ...
        >
          ...
          <span style={{ color: "#94a3b8", fontSize: 11 }}>
            {(meta.urgency_score * 100).toFixed(0)}%
          </span>
        </div>
        <div style={{ background: "#1e293b", borderRadius: 99, height: 6 }}>
          <div
            style={{
              height: 6,
              borderRadius: 99,
              width: `${meta.urgency_score * 100}%`,
              background:
                meta.urgency_score > 0.7
                  ? "#ef4444"
                  : meta.urgency_score > 0.4
                  ? "#f59e0b"
                  : "#10b981",
...
export default function App() {
  ...
  async function handleSubmit() {
    ...
    try {
      const goalAnalysis = await goal_analyzer(
        inputs.goal,
        inputs.deadline,
        days_rem
      );
      const routine = routine_optimizer(inputs, goalAnalysis, days_rem);
      const aiDay = await generate_routine_with_ai(
        inputs,
        goalAnalysis,
        routine,
        1
  ...
  async function handleAdherence() {
    ...
    // Adapt
    const adaptedRoutine = adaptive_scheduler(
      simState.routine,
      adherence,
      missed,
      simState.days_rem
    );
    const aiAdapt = await adapt_plan_with_ai(
      simState.inputs,
      adaptedRoutine,
      adherence,
      missed,
    ...
    const newAiDay = await generate_routine_with_ai(
      simState.inputs,
      simState.goalAnalysis,
      newRoutine,
      simState.day + 1
    );
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                ...
                {["morning", "evening", "flexible"].map((t) => (
                  <button
                    ...
                    style={{
                      ...
                      border: "1px solid",
                      borderColor:
                        form.workout_timing === t ? "#6366f1" : "#1e293b",
                      background:
                        form.workout_timing === t ? "#6366f122" : "#0f172a",
            ...
            <button
              ...
              style={{
                ...
                border: "none",
                background:
                  form.goal && form.deadline
                    ? "linear-gradient(135deg, #6366f1, #8b5cf6)"
                    : "#1e293b",
                color: form.goal && form.deadline ? "#fff" : "#475569",
                fontSize: 15,
                fontWeight: 700,
                cursor: form.goal && form.deadline ? "pointer" : "not-allowed",
                fontFamily: "'Syne', sans-serif",
                letterSpacing: 0.5,
                transition: "all 0.2s",
                boxShadow:
                  form.goal && form.deadline ? "0 0 30px #6366f144" : "none",
              }}
            >
          ...
          >
            ...
            {simHistory.length > 0 && (
              <div style={{ display: "flex", gap: 12 }}>
                ...
                >
                  <div
                    style={{ color: "#f59e0b", fontSize: 22, fontWeight: 800 }}
                  >
                    🔥 {trackerStats.streak}
                  </div>
                ...
                >
                  <div
                    style={{ color: "#10b981", fontSize: 22, fontWeight: 800 }}
                  >
              ...
              >
                {simHistory.map((h, i) => (
                  <div
                    ...
                    style={{
                      background:
                        h.adherence >= 0.8 ? "#10b98122" : "#ef444422",
                      border: `1px solid ${
                        h.adherence >= 0.8 ? "#10b98144" : "#ef444444"
                      }`,
                      borderRadius: 8,
                      padding: "6px 12px",
                      fontSize: 12,
            ...
            {simState?.aiDay?.aiAdapt && (
              ...
              >
                ...
                <div style={{ color: "#34d399", fontSize: 13 }}>
                  💬 {simState.aiDay.aiAdapt.motivation_message}
                </div>
